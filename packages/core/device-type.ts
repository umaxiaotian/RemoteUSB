...
export function detectDeviceType(name: string, vid = '', pid = '', usbClass = ''): RemoteUsbDevice['type'] {
  if (/st-?link|jtag|debug probe/i.test(name) || (vid === '0483' && ['3748', '374b'].includes(pid))) return 'Debug Probe';
  if (/serial|arduino|esp32|ftdi|ch340|cdc acm/i.test(name) || ['1a86', '0403', '2341', '10c4'].includes(vid) || usbClass === '02') return 'Serial';
