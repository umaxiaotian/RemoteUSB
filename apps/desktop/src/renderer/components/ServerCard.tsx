...
export function ServerCard({
  ...
  return (
    <article aria-label={server.name}>
      <Card size="small">
        <Flex gap={16} align="center" wrap>
          ...
          <div className="grow">
            ...
            <Badge
              status={
                error
                  ? "error"
                  : server.enabled && server.lastSeenAt
                    ? "success"
                    : "default"
              }
              text={t(
          ...
          <Flex gap={8} align="center" wrap>
            <Switch
              ...
              onChange={onToggle}
            />
            <Button onClick={onTest}>{t("Test Connection")}</Button>
            <Button
              icon={<Pencil size={15} />}
              aria-label={t("Edit") + " " + server.name}
              onClick={onEdit}
            />
            <Popconfirm title={t("removeConfirm")} onConfirm={onRemove}>
              <Button
                danger
                icon={<Trash2 size={15} />}
        ...
        {error && <Alert type="error" title={t(error.code)} />}
      </Card>
    </article>
  );
}
