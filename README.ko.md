...
NSIS 설치 프로그램이 `release/RemoteUSB-Setup-0.1.5.exe`에 생성됩니다. RemoteUSB 릴리스 빌드는 기본적으로 서명되지 않습니다.
...
- **수동 실행:** Actions → Release → Run workflow에서 배포할 브랜치를 선택하고 `package.json`과 일치하는 태그(예: `v0.1.5`)를 입력합니다. 검증 성공 후 테스트한 커밋에 태그를 만들고 릴리스를 게시합니다.
