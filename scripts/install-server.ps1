...
try {
    ...
    if ($child.ExitCode -eq 3010) {
        ...
        [Windows.Forms.MessageBox]::Show($(if ((Get-UICulture).TwoLetterISOLanguageName -eq 'ja') { 'usbipd-win を導入しました。セットアップを完了するには Windows を再起動してください。' } else { 'usbipd-win installed. Restart Windows to finish setup.' }), 'RemoteUSB') | Out-Null
