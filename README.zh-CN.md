...
生成的 NSIS 安装程序位于 `release/RemoteUSB-Setup-0.1.5.exe`。RemoteUSB 发布构建默认未签名。
...
- **手动运行：** 在 Actions → Release → Run workflow 中选择待发布分支，输入与 `package.json` 一致的标签，例如 `v0.1.5`。验证成功后，在已测试的提交上创建标签并发布 Release。
